...
export const AccountPage = () => {
  ...
  const updateAllRooms = () => {
    const result = AllRoomsAssociated(account.access_token);
    result.then(res => {
  ...
  useEffect(() => {
    const result = GetAccountInfo(account.username, account.access_token);
    result.then(res => {
      if (res.error.status !== 200) {
        history.push('/?error=' + res.error.message);
        return;
    ...
    <>
      ...
      <div className='account-wrapper'>
        ...
        <div className='account-rooms'>
          ...
          <img
            className='account-rooms-create' src='/Create.png'
